...
class BehaviorCounterApp:
    def __init__(self, master):
        ...
        self.behaviors = ['Rearing', 'Digging', 'Grooming', 'Social avoidance']
        ...
        tk.Button(master, text="Save Results", command=self.save_results, bg='lightgreen').grid(row=8, column=0, columnspan=2, pady=10)
        tk.Button(master, text="Reset", command=self.reset_all, bg='salmon').grid(row=8, column=2, columnspan=2, pady=10)
        ...
        try:
            with open(filepath, 'w', newline='') as csvfile:
                ...
                for behavior in self.behaviors:
                    freq = self.frequency_counts[behavior]
                    if freq == 0 and behavior == 'Social avoidance':
                        freq = 'NaN'
                    writer.writerow([
                        behavior,
                        freq,
    ...
    def on_key_press(self, event):
        ...
        if key == 'q':
            self.increment_frequency('Digging')
        elif key == 'w':
            self.increment_frequency('Grooming')
        elif key == 'p':
            self.increment_frequency('Rearing')
        elif key == 'o':
            self.increment_frequency('Social avoidance')
        elif key == 'a':
            self.start_hold('Digging')
        elif key == 's':
            self.start_hold('Grooming')
        elif key == 'l':
            self.start_hold('Rearing')
        elif key == 'space':
            self.start_hold('Social avoidance')
    ...
    def on_key_release(self, event):
        ...
        if key == 'a':
            self.end_hold('Digging')
        elif key == 's':
            self.end_hold('Grooming')
        elif key == 'l':
            self.end_hold('Rearing')
        elif key == 'space':
            self.end_hold('Social avoidance')
